...
Original file is located at
    https://colab.research.google.com/drive/1Wk8Fq5X1huzqWaI-HL_7IOpjKeROAXSg
"""
...
df_core.rename(columns={df_core.columns[0]: "Taxon", "Prevalence": "Core Microbiome"}, inplace=True)
...
# Remove any duplicate Taxa
df_merged = df_merged.drop_duplicates(subset="Taxon")
...
selected = df_merged[
    (df_merged["LDAscore"] >= lda_cutoff) &
    (df_merged["Pvalues"] <= pval_cutoff) &
    (df_merged["Mean Abundance (%)"] >= abundance_cutoff) &
    (df_merged["Core Microbiome"] >= core_cutoff)
].drop_duplicates(subset="Taxon")

# Save selected taxa
selected.to_csv("selected_taxa.csv", index=False)

# Save excluded taxa
excluded = df_merged[~df_merged["Taxon"].isin(selected["Taxon"])]
excluded = excluded.drop_duplicates(subset="Taxon")
